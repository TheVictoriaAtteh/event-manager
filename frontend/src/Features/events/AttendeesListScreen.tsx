...
export const AttendeesListScreen: React.FC<AttendeesListScreenProps> = ({
  ...
  const load = useCallback(
    async (search?: string) => {
      ...
      setError("");
      try {
        const res = await attendeesApi.list(eventId, { search: search || undefined, take: 100 });
  ...
  // The external API supports server-side name/email searching.
  useEffect(() => {
    const timer = setTimeout(() => void load(searchTerm.trim() || undefined), 300);
    ...
    <div className="bg-dot-grid min-h-screen text-[var(--text-primary)] font-sans">
      <div className="max-w-[1500px] mx-auto px-6 sm:px-10 py-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-[var(--text-secondary)] mt-1">{total} attendee{total === 1 ? "" : "s"} registered for this event.</p>
        </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-[var(--border-default)] bg-[var(--bg-surface)]">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {!loading && attendees.map((attendee) => (
                  <tr key={attendee.id} className="border-b border-[var(--border-subtle)] last:border-0 hover:bg-[var(--hover-surface)]">
                    <td className="p-4">
                      <div className="font-medium text-[var(--text-primary)]">{attendee.name}</div>
                      <div className="text-xs text-[var(--text-secondary)]">{attendee.email}</div>
                    </td>
                    <td className="p-4">
                      {attendee.checkIn ? (
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold bg-sky-500/10 border border-sky-500/20 text-sky-500"><UserCheck className="w-3.5 h-3.5" /> Checked In</span>
                      ) : (
                    ...
                    <td className="p-4 text-right">
                      <button onClick={() => void handleRemove(attendee.id, attendee.name)} disabled={removingId === attendee.id} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-rose-500 bg-rose-500/10 border border-rose-500/20 hover:bg-rose-500/20 transition-all cursor-pointer disabled:opacity-60">
                        <Trash2 className="w-3.5 h-3.5" /> {removingId === attendee.id ? "Removing…" : "Remove"}
                      </button>
                    </td>
                  </tr>
                ))}
          ...
          {loading && <div className="py-16 flex flex-col items-center gap-3 text-[var(--text-secondary)]"><Loader2 className="w-6 h-6 animate-spin" /><p className="text-sm">Loading attendees…</p></div>}
          {!loading && !error && attendees.length === 0 && (
            <div className="py-16 text-center">
              ...
              <div className="mt-4 flex items-center justify-center gap-3">
                <button onClick={onAddAttendee} className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-emerald-950 text-sm font-semibold cursor-pointer"><UserPlus className="w-4 h-4" /> Add Attendee</button>
                <button onClick={onUploadAttendees} className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-[var(--bg-input)] border border-[var(--border-default)] text-sm font-medium cursor-pointer"><Upload className="w-4 h-4" /> Upload CSV</button>
              </div>
            </div>
          )}
        </div>
